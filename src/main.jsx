...
const programs = [
  {
    ...
    accent: 'Fondations atelier',
    description:
      'Moteur, transmission, freinage, suspension et entretien courant sur de vraies voitures.',
    duration: '2-3 ans',
    price: '150 000 FCFA',
    icon: Wrench,
  },
  {
    title: 'Mécatronique',
    accent: 'Systèmes modernes',
    description:
      'La mécanique rencontre l’électronique embarquée: capteurs, calculateurs et diagnostic logique.',
    duration: '2-3 ans',
    price: 'Sur devis',
    icon: CircuitBoard,
  },
...
const levels = ['Aucun', 'CEP', 'BEPC', 'Probatoire', 'Baccalauréat', 'BTS / Licence et +'];
const experiences = [
...
const sources = ['TikTok', 'Facebook', 'WhatsApp', 'Recommandation'];

function App() {
  const [page, setPage] = useState(window.location.hash === '#inscription' ? 'register' : 'home');

  useEffect(() => {
    const syncPage = () => {
      setPage(window.location.hash === '#inscription' ? 'register' : 'home');
    };
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-neutral-950">
      <Header page={page} onHome={goHome} onRegister={goRegister} />
      {page === 'home' ? <HomePage onRegister={goRegister} /> : <RegisterPage onHome={goHome} />}
      <Footer onRegister={goRegister} />
    </div>
  );
}

function Header({ page, onHome, onRegister }) {
  return (
      ...
      <section className="perf-hero relative flex items-center overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-5 py-16 sm:px-8">
          ...
          <div className="mt-9 flex flex-wrap gap-4">
            ...
            <a className="btn-skew g" href={`https://wa.me/${WA_NUMBER}`} target="_blank">
              <span>WhatsApp</span>
            </a>
          </div>
  ...
  return (
    <section className={`rounded-[8px] border bg-white p-5 shadow-sm sm:p-6 ${invalid ? 'border-red-300' : 'border-neutral-200'}`}>
      <div className="mb-5 flex items-center gap-3">
        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-neutral-200 text-sm font-black text-red-700">
          {number}
        </span>
  ...
  return (
    <label className={`block ${className}`}>
      ...
      <input
        ...
        inputMode={inputMode}
        className={`h-12 w-full rounded-[8px] border bg-stone-50 px-4 text-base font-semibold outline-none transition focus:border-red-600 focus:bg-white ${
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-5 sm:px-8 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-4 max-w-sm text-sm font-medium leading-6 text-neutral-600">
            Centre de formation en mécanique automobile à Yaoundé. De la passion à la maîtrise.
          </p>
        </div>
